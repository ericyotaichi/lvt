...
const form = useForm({
  ...
  product_id: props.application?.product_id ?? '',
  status: props.application?.status ?? 'draft',
  sort: props.application?.sort ?? 0,
  cover: null,
})
...
const submit = () => {
  ...
  if (props.mode === 'create') {
    ...
    form.post(route('admin.applications.update', props.application.id), {
      forceFormData: true,
      onError: (errs) => { if (isDev) console.warn('[SUBMIT][ERROR]', errs) },
    })
  }
...
<template>
  <Head :title="props.mode === 'create' ? '新增應用場域' : '編輯應用場域'" />

  <div class="max-w-5xl mx-auto">
    <div class="flex items-center justify-between gap-4 mb-6">
      <div>
        <h1 class="text-2xl font-bold">
          {{ props.mode === 'create' ? '新增應用場域' : '編輯應用場域' }}
        </h1>
        <p class="text-gray-600 mt-1">指定對應產品，讓前台分類與錨點同步。</p>
      </div>
    ...
    <form @submit.prevent="submit" class="space-y-6">
      <section class="rounded-2xl border bg-white p-6">
        ...
        <div class="grid gap-4 md:grid-cols-2">
          <div>
            <label class="block text-sm font-medium">應用場域名稱 (中文) *</label>
          ...
          <div>
            <label class="block text-sm font-medium">應用場域名稱 (English)</label>
          ...
          <div>
            <label class="block text-sm font-medium">對應產品</label>
            <select v-model="form.product_id" class="mt-1 w-full rounded-xl border px-3 py-2">
              <option value="">未分類</option>
              <option v-for="product in props.products" :key="product.id" :value="product.id">
                {{ product.title }}
              </option>
            </select>
            <p v-if="form.errors.product_id" class="text-sm text-red-600">{{ form.errors.product_id }}</p>
          </div>
          <div>
      ...
      <section class="rounded-2xl border bg-white p-6">
        ...
        <div v-if="coverPreview || (props.application?.cover_url && !form.cover)" class="mt-3">
          <p class="text-sm text-gray-500 mb-2">{{ coverPreview ? '預覽' : '目前封面' }}：</p>
          <div class="relative inline-block">
            <img
              :src="coverPreview || props.application.cover_url"
              alt="Cover preview"
