...
const confirmDelete = (id, title) => {
  const message = locale.value === 'en'
    ? `Are you sure you want to delete "${title || 'this application'}"? This action cannot be undone.`
    : `確定要刪除「${title || '此應用場域'}」嗎？此操作無法復原。`
  if (confirm(message)) {
    deletingId.value = id
...
<template>
  ...
  <div class="max-w-7xl mx-auto">
    <header class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
      <div>
        <h1 class="text-2xl font-bold">應用場域管理</h1>
        <p class="text-gray-600 mt-1">新增、編輯應用場域內容，並指定對應產品。</p>
      </div>
      <Link
    ...
    <div class="overflow-x-auto rounded-2xl border bg-white">
      <table class="min-w-full text-sm">
        <thead class="bg-gray-50">
          <tr>
            <th class="text-left px-4 py-3 font-medium">應用場域</th>
            <th class="text-left px-4 py-3 font-medium">對應產品</th>
        ...
        <tbody>
          <tr v-for="row in items.data" :key="row.id" class="border-t">
            ...
            <td class="px-4 py-3">
              <div class="text-sm text-gray-600">{{ row.product_title || '未分類' }}</div>
            </td>
            <td class="px-4 py-3">
              <div
            ...
            <td class="px-4 py-3">
              <div class="flex gap-2">
                <Link
                  :href="route('admin.applications.edit', row.id)"
